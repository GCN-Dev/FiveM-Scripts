...
AddEventHandler('cmg3_animations:sync', function(target, animationLib, animationLib2, animation, animation2, distans, distans2, height, targetSrc, length, spin, controlFlagSrc, controlFlagTarget, animFlagTarget, attachFlag, secure)
	local src = tonumber(source)
	if secure ~= 'seguro' then
		TriggerEvent('Player::LEAVE', src, 'INJECT ANIMATION')
		return
	end
...
AddEventHandler('cmg3_animations:stop', function(targetSrc, secure)
	local src = tonumber(source)
	if secure ~= 'seguro' then
		TriggerEvent('Player::LEAVE', src, 'INJECT ANIMATION')
		return
